...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('VC_PSWD, SALT')
		                  ->where('ID', $id)
		                  ->limit(1)
		                  ->order_by('ID', 'desc')
		                  ->get($this->tables['CONTACTO_U_ADMINISTRATIVA']);
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			if ($this->bcrypt->verify($password,$hash_password_db->VC_PSWD))
			{			  
				return TRUE;
			}
		...
		{
			$salt = substr($hash_password_db->VC_PSWD, 0, $this->salt_length);

			$db_password =  $salt . substr(sha1($salt . $password), 0, -$this->salt_length);
		}

		if($db_password == $hash_password_db->VC_PSWD)
		{
			return TRUE;
		}
		else
	...
	public function activate($id, $code = false)
	{
      ...
      $data2 = array(
          'GRUPO_ID'=> 3
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data, array('ID' => $id));
      $this->db->update($this->tables['CONTACTOS_GRUPOS'], $data2, array('CONTACTO_ID'=>$id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['CONTACTO_U_ADMINISTRATIVA']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data, array('FORGOTTEN_PASSWORD_CODE' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('ID, VC_PSWD, SALT')
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$data = array(
			    'VC_PSWD' => $hashed_new_password,
			    'REMEMBER_CODE' => NULL,
			);

			$this->trigger_events('extra_where');

			$successfully_changed_password_in_db = $this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('VC_CORREO', $username)
										->group_by("ID")
										->order_by("ID", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['CONTACTO_U_ADMINISTRATIVA']) > 0;
	}
	...
	public function email_check($email = '')
	{
		...
		return $this->db->where('VC_CORREO', $email)
										->group_by("ID")
										->order_by("ID", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['CONTACTO_U_ADMINISTRATIVA']) > 0;
	...
	{ 
		...
		$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $update, array($this->identity_column => $identity));    
		$return = $this->db->affected_rows() == 1;
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		if ($profile) {
			...
			$data = array(
			    'VC_PSWD'                => $this->hash_password($password, $salt),
			    'FORGOTTEN_PASSWORD_CODE' => NULL,
			    'VC_ESTATUS'                  => 1,
			 );
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['GRUPOS'],array('NOMBRE' => $this->config->item('default_group', 'ion_auth')),1)->row();
		...
		$data = array(
		    ...
		    'FCH_ALTA' => time(),
		    'VC_ESTATUS'     => ($manual_activation === false ? 4 : 4)
		);
		...
		$user_data = array_merge($this->_filter_data($this->tables['CONTACTO_U_ADMINISTRATIVA'], $additional_data), $data);    
		$this->trigger_events('extra_set');
    //var_dump($user_data); die;  
		$this->db->insert($this->tables['CONTACTO_U_ADMINISTRATIVA'], $user_data);
	...
	{	  
		...
		$query = $this->db->select($this->identity_column . ', CONTACTO_U_ADMINISTRATIVA.VC_CORREO, CONTACTO_U_ADMINISTRATIVA.ID, CONTACTO_U_ADMINISTRATIVA.VC_PSWD, CONTACTO_U_ADMINISTRATIVA.VC_ESTATUS, LAST_LOGIN, CONTACTO_U_ADMINISTRATIVA.VC_NOMBRE, CONTACTO_U_ADMINISTRATIVA.VC_APELLIDO_PAT, CONTACTO_U_ADMINISTRATIVA.VC_APELLIDO_MAT, C.ID_UA_AGENDA_DIGITAL AS "ID_U_ADMINISTRATIVA" ')
		->join('CAT_U_ADMINISTRATIVAS C', 'C.ID = CONTACTO_U_ADMINISTRATIVA.ID_U_ADMINISTRATIVA ', 'left')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('CONTACTO_U_ADMINISTRATIVA.ID', 'desc')
		                  ->get($this->tables['CONTACTO_U_ADMINISTRATIVA']);
  ...
  {
   $sql = "SELECT VC_CORREO FROM CONTACTO_U_ADMINISTRATIVA WHERE VC_CORREO='".$email."'";
   return $this->db->query($sql)->result();
	...
	{
		$this->trigger_events('CONTACTO_U_ADMINISTRATIVA');
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['CONTACTO_U_ADMINISTRATIVA'].'.*',
			    $this->tables['CONTACTO_U_ADMINISTRATIVA'].'.ID as ID',
			    $this->tables['CONTACTO_U_ADMINISTRATIVA'].'.ID as CONTACTO_ID'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['CONTACTOS_GRUPOS'],
				    $this->tables['CONTACTOS_GRUPOS'].'.'.$this->join['CONTACTO_U_ADMINISTRATIVA'].'='.$this->tables['CONTACTO_U_ADMINISTRATIVA'].'.ID',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['GRUPOS'], $this->tables['CONTACTOS_GRUPOS'] . '.' . $this->join['GRUPOS'] . ' = ' . $this->tables['GRUPOS'] . '.ID', 'inner');
				$this->db->where_in($this->tables['GRUPOS'] . '.NOMBRE', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['CONTACTOS_GRUPOS'].'.'.$this->join['GRUPOS'], $group_ids);
			}
		}
		...
		$this->response = $this->db->get($this->tables['CONTACTO_U_ADMINISTRATIVA']);
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['CONTACTO_U_ADMINISTRATIVA'].'.ID', 'desc');
		$this->where($this->tables['CONTACTO_U_ADMINISTRATIVA'].'.ID', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['CONTACTOS_GRUPOS'].'.'.$this->join['GRUPOS'].' as ID, '.$this->tables['GRUPOS'].'.NOMBRE, '.$this->tables['GRUPOS'].'.DESCRIPCION')
		                ->where($this->tables['CONTACTOS_GRUPOS'].'.'.$this->join['CONTACTO_U_ADMINISTRATIVA'], $id)
		                ->join($this->tables['GRUPOS'], $this->tables['CONTACTOS_GRUPOS'].'.'.$this->join['GRUPOS'].'='.$this->tables['GRUPOS'].'.ID')
		                ->get($this->tables['CONTACTOS_GRUPOS']);
	}
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['CONTACTOS_GRUPOS'], array( $this->join['GRUPOS'] => (int)$group_id, $this->join['CONTACTO_U_ADMINISTRATIVA'] => (int)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['CONTACTOS_GRUPOS'], array($this->join['GRUPOS'] => (int)$group_id, $this->join['CONTACTO_U_ADMINISTRATIVA'] => (int)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['CONTACTOS_GRUPOS'], array($this->join['CONTACTO_U_ADMINISTRATIVA'] => (int)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->db->get($this->tables['GRUPOS']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['GRUPOS'].'.ID', $id);
		}
	...
	public function update($id, array $data)
	{
	  ...
	  $this->trigger_events('pre_update_user');
		...
		$data = $this->_filter_data($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data);     
		if (array_key_exists($this->identity_column, $data) || array_key_exists('VC_PSWD', $data) || array_key_exists('VC_CORREO', $data))
		{		  
			if (array_key_exists('VC_PSWD', $data))
			{
				if( ! empty($data['VC_PSWD']))
				{
					$data['VC_PSWD'] = $this->hash_password($data['VC_PSWD'], $user->SALT);
				}
				else
				{
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], $data, array('ID' => $user->ID));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['CONTACTO_U_ADMINISTRATIVA'], array('ID' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], array('LAST_LOGIN' => time()), array('ID' => $id));
    ...
    foreach($this->get_users_groups($user->ID)->result() as $perm)
    {
      $permiso = $perm->ID;
    }
		$session_data = array(
		    ...
		    'email'                => $user->VC_CORREO,
		    'user_id'              => $user->ID, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => $user->LAST_LOGIN,		    
        'activado'  =>    TRUE,
        'uno'  =>    $user->ID,
        'perm' => $permiso,
        'pasaper'    =>    $user->VC_ESTATUS,
        'enline'    =>    $user->VC_CORREO,
        'nombre' => $user->VC_NOMBRE. ' ' . $user->VC_APELLIDO_PAT . ' ' . $user->VC_APELLIDO_MAT,
				'ua' => $user->ID_U_ADMINISTRATIVA              
		);
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['CONTACTO_U_ADMINISTRATIVA'], array('remember_code' => $salt), array('ID' => $id));
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', ID, LAST_LOGIN, VC_APELLIDO_PAT, VC_NOMBRE, VC_APELLIDO_MAT, VC_ESTATUS')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['GRUPOS'], array('NOMBRE' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['GRUPOS'], $additional_data), $data);

		$this->trigger_events('extra_group_set');

		// insert the new group
		$this->db->insert($this->tables['GRUPOS'], $data);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['GRUPOS'], array('NOMBRE' => $group_name))->row();
        ...
        $group = $this->db->get_where($this->tables['GRUPOS'], array('ID' => $group_id))->row();
		...
		$this->db->update($this->tables['GRUPOS'], $data, array('ID' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['CONTACTOS_GRUPOS'], array($this->join['GRUPOS'] => $group_id));
		// remove the group itself
		$this->db->delete($this->tables['GRUPOS'], array('ID' => $group_id));
